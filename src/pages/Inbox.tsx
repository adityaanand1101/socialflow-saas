...
export function Inbox() {
  ...
  const handleReply = async () => {
    ...
    try {
      const token = await getToken()
      const res = await apiFetch(`/api/inbox/comments/${selectedComment.id}/reply`, {
        method: 'POST',
  ...
  const uniquePlatforms = [...new Set(comments.map(c => c.platform))]
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {totalUnread > 0
              ? `${totalUnread} unread comment${totalUnread !== 1 ? 's' : ''} across ${uniquePlatforms.length} platform${uniquePlatforms.length !== 1 ? 's' : ''}`
              : 'All caught up'}
          </p>
        </div>
        <Button variant="ghost" size="sm" className="gap-2 text-muted-foreground hover:text-white" onClick={handleRefresh} disabled={refreshing}>
        ...
        <div>
          ...
          <div className="bg-white/[0.02] rounded-xl border border-white/[0.06] overflow-hidden">
            <div className="px-5 py-4 border-b border-white/[0.06]">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center text-white font-bold text-sm">
                  {selectedComment.fromName[0]}
                </div>
                <div>
                  <p className="text-sm font-semibold text-white">{selectedComment.fromName}</p>
                  <p className="text-[11px] text-muted-foreground">@{selectedComment.fromUsername}</p>
                </div>
              </div>
            </div>
            ...
            <div className="px-5 py-4 border-t border-white/[0.06] flex items-center gap-3">
              ...
              />
              <Button size="sm" className="gap-1.5" onClick={handleReply} disabled={sendingReply || !replyText.trim()}>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-white">{comment.fromName}</span>
                        ...
                        <p className={cn("text-sm leading-relaxed", comment.isRead ? "text-white/70" : "text-white")}>
                          {comment.content}
                        </p>
                        {comment.postCaption && (
